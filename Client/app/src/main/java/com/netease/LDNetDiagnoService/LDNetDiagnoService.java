...
public class LDNetDiagnoService extends
  ...
  private static final String TAG = LDNetDiagnoService.class.getSimpleName();
  ...
  public String startNetDiagnosis() {
    ...
    if (_isNetConnected) {
      ...
      // 诊断ping信息, 同步过程
      recordStepInfo("\n开始ping...");
      Log.e(TAG,"z走到这里"+_isNetConnected+_isDomainParseOk+_isSocketConnected);
      if (!(_isNetConnected && _isDomainParseOk && _isSocketConnected)) {// 联网&&DNS解析成功&&connect测试成功
        Log.e(TAG,"应该走到这里");
        System.out.println("应该要走到这里");
